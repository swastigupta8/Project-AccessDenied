...
const NetworkSecurity = () => {
  const [networkData, setNetworkData] = useState({ threat_confidence: 0, status: 'LOW' });
  ...
  const lineColor = networkData.status === 'HIGH' ? '#ef4444' : networkData.status === 'MEDIUM' ? '#eab308' : '#a855f7';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 h-full min-h-0">
          ...
          <div className="lg:col-span-2 bg-neutral-900/30 border border-white/5 rounded-2xl p-5 md:p-6 flex flex-col min-h-[300px] lg:min-h-0">
            <div className="flex justify-between items-center mb-6 shrink-0">
              <h1 className="font-serif text-2xl md:text-3xl text-gray-100">IT Network Monitor</h1>
              ...
              <div className="absolute inset-0">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <Area type="monotone" dataKey="network" stroke={lineColor} strokeWidth={2} fill="url(#netGrad)" isAnimationActive={false} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          <div className="lg:col-span-1 flex flex-col gap-4 md:gap-6 min-h-0">
            <ThreatLevelBox label="IT Network Threat" percentage={networkData.threat_confidence} status={networkData.status} />

            <div className={`shrink-0 border rounded-2xl p-5 ${networkData.status === 'HIGH' ? 'bg-red-950/20 border-red-500/20' : networkData.status === 'MEDIUM' ? 'bg-yellow-950/20 border-yellow-500/20' : 'bg-neutral-900/30 border-white/5'}`}>
              <div className="flex items-center gap-2 mb-3">
                <Zap size={14} className={networkData.status === 'HIGH' ? 'text-red-500' : 'text-purple-500'} />
                <span className="font-mono text-[10px] font-bold tracking-widest text-gray-400 uppercase">Autonomous Intel</span>
              </div>
              <p className={`text-xs leading-relaxed ${networkData.status === 'HIGH' ? 'text-red-200' : networkData.status === 'MEDIUM' ? 'text-yellow-200' : 'text-green-400'}`}>
                {networkData.status === 'HIGH' ? "CRITICAL: Isolation Forest flagged severe anomalous traffic. Isolate subnets immediately." : networkData.status === 'MEDIUM' ? "WARNING: Unusual traffic spikes detected. Monitor endpoints." : "NOMINAL: Network telemetry within baselines."}
              </p>
            </div>
